...
const teamsReducer = (state: { activeTeams: Team[]; suspendedTeams: Team[] }, action: ActionType): { activeTeams: Team[]; suspendedTeams: Team[] } => {
  switch (action.type) {
...
const TeamList: React.FC<TeamListProps> = ({ displayIconsOnly = false }) => {
  ...
  useEffect(() => {
    ...
    if (loadedActiveTeams) {
      teamsDispatch({ type: 'SET_ACTIVE_TEAMS', payload: JSON.parse(loadedActiveTeams) });
    }

    if (loadedSuspendedTeams) {
      teamsDispatch({ type: 'SET_SUSPENDED_TEAMS', payload: JSON.parse(loadedSuspendedTeams) });
    }
  ...
  const handleTeamNameClick = async (members: DocumentReference[]) => {
    ...
    try {
      ...
      if (userInfos.length > 0) {
        ...
        dispatch({ type: 'SET_SELECTED_MEMBERS', payload: formattedUserInfos.join(', ') });
      } else {
        console.error('No valid user document found for the given references.');
      }
    } catch (error) {
