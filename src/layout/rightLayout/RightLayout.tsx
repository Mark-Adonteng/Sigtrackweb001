...
const RightLayout: React.FC<RightLayoutProps> = ({
  ...
  return (
    <div
      className={`third-section ${
        ...
        transition-width flex justify-center items-center max-h-96 bg-custom-bg
        transition-width duration-100 ease-in-out overflow-${
          isNarrowed ? 'hidden' : 'visible'
        } ${isNarrowed ? 'w-10' : 'w-300'} ${isNarrowed ? 'h-auto' : 'h-full'}`}
    >
      <div>
        <button
  ...
  <IsRightLayoutNarrowedContext.Provider value={isNarrowed}>
    <div className="ml-36 space-y-4">
      ...
      {!isNarrowed && <div style={{ position: 'absolute' }}>
      <IsRightLayoutNarrowedContext.Provider value={isNarrowed}>
          {children}
          </IsRightLayoutNarrowedContext.Provider>
        </div>}
...
//     <TeamItem
//       teamItemName={teamItemName}
//       icon={icon}
//       duration={duration}
//       isCollapsed={isCollapsed}
//       backgroundColor={backgroundColor}
//     />
//     <h1 style={{ color: 'red' }}>mhb</h1>
//   </div>
// )}

// {!isNarrowed && <div style={{ position: 'absolute' }}>{children}</div>}
// </div>